...
function killProc(p, name) {
  ...
  try {
    ...
    console.error(`[VNC] erro ao encerrar ${name}:`, e?.message || e);
  }
}
...
app.post("/_admin/vnc/start", (req, res) => {
  ...
  if (status().running) {
    return res.json({ ok: true, already_running: true, ...status() });
  }

  try {
    console.log("[ADMIN] start VNC solicitado");
    startVncStack();
    return res.json({ ok: true, started: true, ...status() });
  } catch (e) {
...
app.post("/_admin/vnc/stop", (req, res) => {
  if (!requireAdmin(req, res)) return;
  stopVncStack();
  return res.json({ ok: true, stopped: true, ...status() });
});
